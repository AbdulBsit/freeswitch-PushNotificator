...
static int do_curl(switch_event_t *event, profile_t *profile)
{
        ...
        if (!strcasecmp(method, "post")) {
				...
				if (post_data != profile->post_data_template) {
                                        ct = switch_mprintf("Content-Type: application/json");
            ...
            if (content_type) {
				ct = switch_mprintf("Content-Type: %s", content_type);
				headers = switch_curl_slist_append(headers, ct);
				switch_safe_free(ct);
			}
		} else {
...
static switch_bool_t mod_apn_send(switch_event_t *event, profile_t *profile)
{
        ...
        if (!profile) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "EMITRR. APN profile not found\n");
                return ret;
        }
...
static void db_get_tokens_array(char *user, char *realm, char *type, callback_t *cbt)
{
        ...
        if (zstr(user) || zstr(realm)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. No parameters for get token. user: '%s', realm: '%s'\n", user, realm);
                return;
        }
...
static void push_event_handler(switch_event_t *event)
{
        ...
        if (zstr(type) || zstr(user) || zstr(realm)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. No parameters, data: '%s', type: '%s', user: '%s', realm: '%s'\n", payload, type, user, realm);
                goto end;
        }
        ...
        if ((size = cJSON_GetArraySize(cbt.array)) == 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. No one token found for type: '%s', user: '%s', realm: '%s'\n", type, user, realm);
                goto end;
        }
        ...
        if (!zstr(uuid) && switch_event_create_subclass(&res_event, SWITCH_EVENT_CUSTOM, "mobile::push::response") == SWITCH_STATUS_SUCCESS) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Fire event mobile::push::response with ID: '%s' and result: '%s'\n", uuid, res ? "sent" : "notsent");
...
static void originate_register_event_handler(switch_event_t *event)
{
        ...
        if (!zstr(update_reg) && switch_true(update_reg)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Update existing registration, skip originate\n");
                return;
        }
        ...
        if (zstr(event_username) || zstr(event_realm) || zstr(event_call_id) || zstr(event_profile) ||  zstr(event_contact) || zstr(domain_name) || zstr(dial_user)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. No parameter for originate call via sofia::register\n");
                return;
        }
        ...
        if (zstr(dest)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. No destination contact data string\n");
                goto end;
        }
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Try originate to '%s' (by registration event) for callId '%s'\n", destination, originate_data->x_call_id);
...
static void register_event_handler(switch_event_t *event)
{
        ...
        voip_token = switch_event_get_header(event, "voip_token");
        platform = switch_event_get_header(event, "platform");
        ...
        if (zstr(event_user) || zstr(event_realm)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. No parameter\n");
                goto end;
        }
        ...
        if (!zstr(voip_token)) {
                ...
                if (!zstr(voip_count) && strtol(voip_count, NULL, 10) == 0) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Add new VoIP token: '%s' to push_tokens for user %s@%s and application: %s\n", voip_token, event_user, event_realm, app_id);
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. VoIP token: '%s' for user %s@%s and application: %s already exists\n", voip_token, event_user, event_realm, app_id);
        ...
        if (!zstr(im_token)) {
                ...
                if (!zstr(im_count) && strtol(im_count, NULL, 10) == 0) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Add new IM token: '%s' to push_tokens for user %s@%s and application: %s\n", im_token, event_user, event_realm, app_id);
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. IM token: '%s' for user %s@%s and application: %s already exists\n", im_token, event_user, event_realm, app_id);
			 ...
			 switch_call_cause_t *cancel_cause)
{
    ...
    //====== Add By Bhavik
    // Also check var_event for video indicators
    if (var_event) {
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "APN originate_timeout Is ::: -----%i\n", timelimit_sec);
    ...
    originate_data.x_call_id = switch_core_strdup(pool, x_call_id);
    ...
    if (!var_event || (var_event && (!(var_val = switch_event_get_header(var_event, "enable_send_apn")) || zstr(var_val) || switch_true(var_val)))) {
      if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, "mobile::push::notification") == SWITCH_STATUS_SUCCESS) {
        ...
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "from", cid_num_override);

        switch_event_add_body(event, "{\"content-available\":true,\"custom\":["
          "{\"name\":\"content-message\",\"value\":\"incoming call from Emitrr\"},"
          "{\"name\":\"to-user\",\"value\":\"%s\"},"
          "{\"name\":\"from-user\",\"value\":\"%s\"},"
          "{\"name\":\"type\",\"value\":\"voip\"},"
          "{\"name\":\"uuid\",\"value\":\"%s\"},"
          "{\"name\":\"user\",\"value\":\"%s\"},"
          "{\"name\":\"realm\",\"value\":\"%s\"}"
          "]}",
          user, cid_num_override, apn_response.uuid, user, domain);

        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Fire event APN for User: %s@%s\n", user, domain);
    ...
    while (current_timelimit > 0) {
      ...
      if (wait_any_register != SWITCH_TRUE) {
        ...
        if (apn_response.state == MOD_APN_NOTSENT) {
          ...
          switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "EMITRR. Event APN don't sent to %s@%s, so stop wait for incoming register\n", user, domain);
          break;
        }
        switch_mutex_unlock(apn_response.mutex);
      }
      ...
      if (channel && !switch_channel_ready(channel)) {
      	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Skip originate, Channel Not Ready '%s'\n", x_call_id);
        break;
      }

      if (cancel_cause && *cancel_cause > 0) {
      	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Skip originate, cancel_cause > 0 '%d' for '%s'\n", *cancel_cause, x_call_id);
        break;
      }
